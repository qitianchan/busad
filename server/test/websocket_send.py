...
send_data = {"cmd": "tx", "EUI": "BE7A0000000005D2", "port": 1, "encdata": "5659", 'seqno': seqno}
...
#                 data = DATA[i]
#                 data = LoRaCrypto().payload_encrypt(data, APPSKEY, ADDRESS, DIRECTION, SEQUENCE_COUNTER)
#                 send_data['data'] = data
#                 # send_data['data'] = data
#                 res = ws.send(json.dumps(send_data))
#                 print data
...
def send_file(file_name, ws, eui):
    ...
    with open(file_name, 'rb') as f:
        while True:
            ...
            if not chunk:
                break
            send_data['data'] = chunk.encode('hex')
            send_data['seqno'] = seqno
            print seqno
            seqno += 1
            send_data['encdata'] = LoRaCrypto().payload_encrypt(chunk.encode('hex'), APPSKEY, ADDRESS, DIRECTION, seqno)
            print send_data['encdata']
